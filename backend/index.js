...
app.use(cors());
const API_KEY = "AIzaSyBhKoKRm8NLaeKWdEnxy3qwTvYfhsTeZ5E"; // Replace with your actual API key
...
app.post('/signup', async (req, res) => {
  const logdatabase =  client.db("Logs");
const logcollection =  logdatabase.collection("log");
  const { username, email, password } = req.body;
  const existingUser = await logcollection.findOne({ email });
  if (existingUser) {
    return res.status(400).json({ message: 'User already exists' });
  }
  const hashedPassword = await bcrypt.hash(password, 10);
  await logcollection.save({ username, email, password: hashedPassword });
